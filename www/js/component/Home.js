...
console.log( pagenate_display)
    ...
    async exec_wasm_row(items){
        const { wasm_post_row } = wasm_bindgen;
        await wasm_bindgen('./pkg/wasm_cms2_bg.wasm');
        return items.map(function(object, i){
            var json = JSON.stringify( object );
//            console.log( json )
            wasm_post_row("div_post_wrap", String(json) );                
        })
    }    
    tabRow(){
        if(this.state.data.items instanceof Array){
            this.exec_wasm_row( this.state.data.items );
        }        
    }
    handleClickPagenate(){
